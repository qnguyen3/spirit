...
## Comments: none - VERY IMPORTANT

**Write zero comments** in any code you write or edit. This bans all of it:

- `//` and `/* */` and `#` comments
- JSDoc and docstrings of any length
- "Note:", "TODO:", "Important:" comments
- Section headers and decorative separators
- Comments narrating what the next line does
- Comments explaining why _your change_ is correct — that's PR talk, not code

If you feel a comment is needed, the fix is a better name or a smaller function,
not a comment.

**Sole exception:** a hidden constraint or bug workaround that would make a
future reader _break the code_ if they didn't know it. One short line, rare —
most tasks have none. "This explains my change" never qualifies.

**Mandatory self-check before reporting any task done:** run `git diff`, search
the added lines for `//`, `/*`, `#`, and `*`. Every match must either be deleted
or meet the sole exception above. If in doubt, delete it. Do not report the task
finished until this check passes.
...
The GUI desktop app is the `app/` crate on the WarpUI pixel/GPU framework (`warpui`, `crates/warpui_core`): `Element`/`View` layout, GPU/WGSL rendering, mouse input, `.app` bundles. Run with `cargo run` / `./script/run`; verify visually with `computer_use` or the real-display integration framework (`crates/integration`).
...
**GUI rendering** (WarpUI GUI elements):
