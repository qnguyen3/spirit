...
use super::proto::{
    ...
    GitStatusPush, Initialize, InitializeResponse, NavigatedToDirectory,
                ...
                let outcome = match wrapper.message {
                    ...
                    Some(host_scoped_request::Message::ReadFileContext(_)) => {
                        HandlerOutcome::Sync(server_message::Message::Error(ErrorResponse {
                            ...
                            message: "ReadFileContext is not supported by this server".to_string(),
                        }))
                    }
                    ...
                    Some(host_scoped_request::Message::IndexCodebase(_)) => {
                        HandlerOutcome::Sync(server_message::Message::Error(ErrorResponse {
                            code: ErrorCode::InvalidRequest.into(),
                            message: "IndexCodebase is not supported by this server".to_string(),
                        }))
                    }
                    Some(host_scoped_request::Message::DropCodebaseIndex(_)) => {
                        HandlerOutcome::Sync(server_message::Message::Error(ErrorResponse {
                            code: ErrorCode::InvalidRequest.into(),
                            message: "DropCodebaseIndex is not supported by this server"
                                .to_string(),
                        }))
                    }
                    Some(host_scoped_request::Message::GetFragmentMetadataFromHash(_)) => {
                        HandlerOutcome::Sync(server_message::Message::Error(ErrorResponse {
                            code: ErrorCode::InvalidRequest.into(),
                            message: "GetFragmentMetadataFromHash is not supported by this server"
                                .to_string(),
                        }))
                    }
                    ...
                    Some(host_scoped_request::Message::ResyncCodebase(_)) => {
                        HandlerOutcome::Sync(server_message::Message::Error(ErrorResponse {
                            code: ErrorCode::InvalidRequest.into(),
                            message: "ResyncCodebase is not supported by this server".to_string(),
                        }))
                    }
                    Some(host_scoped_request::Message::UploadHandoffSnapshot(_)) => {
                        HandlerOutcome::Sync(server_message::Message::Error(ErrorResponse {
                            code: ErrorCode::InvalidRequest.into(),
                            message: "UploadHandoffSnapshot is not supported by this server"
                                .to_string(),
                        }))
                    }
