...
/// previous_commands can be similar to the user's recently run commands, and next_command can be
/// used to predict the next command.
#[derive(Clone)]
...
pub fn get_similar_history_context(
    ...
    same_commands_from_history
        ...
        .filter_map(|command| {
            ...
            // Get some more commands that came before command so there's additional context
            // before next_command.
...
/// Returns the most recent command with a matching prefix run in the user's current working
/// directory. If no such command exists, returns the most recent command anywhere with a matching
/// prefix.
...
async fn is_arg_valid(
    ...
    cfg_if::cfg_if! {
        if #[cfg(feature = "local_fs")] {
            ...
            let _ = (full_command, ctx, session_env_vars);
            true
        }
    }
}
