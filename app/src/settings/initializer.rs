...
/// User preferences key recording that [`SettingsInitializer::apply_first_run_defaults`] has
/// already run, so a user who has been through it once is never re-defaulted.
const FIRST_RUN_DEFAULTS_APPLIED_KEY: &str = "FirstRunDefaultsApplied";
...
impl SettingsInitializer {
    ...
    /// Applies the defaults that should only take effect for a brand-new install.
    ///
    /// Useful for settings whose desired default has changed since the value in
    /// `define_settings_group!` was chosen, where changing that value would also change the
    /// setting for existing users.
    ///
    /// Runs at most once per install, guarded by [`FIRST_RUN_DEFAULTS_APPLIED_KEY`].
    pub fn apply_first_run_defaults(&self, ctx: &mut ModelContext<Self>) {
        ...
        if first_run_defaults_applied(ctx) {
            return;
        }
        mark_first_run_defaults_applied(ctx);
...
fn first_run_defaults_applied(ctx: &AppContext) -> bool {
    ctx.private_user_preferences()
        .read_value(FIRST_RUN_DEFAULTS_APPLIED_KEY)
        .unwrap_or_default()
        .and_then(|value| serde_json::from_str::<bool>(&value).ok())
        .unwrap_or(false)
}

fn mark_first_run_defaults_applied(ctx: &AppContext) {
    let _ = ctx.private_user_preferences().write_value(
        FIRST_RUN_DEFAULTS_APPLIED_KEY,
