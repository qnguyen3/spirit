...
use crate::pane_group::{
    ...
    Direction as PaneGroupDirection, Direction, FilePane, NewTerminalOptions, PaneGroup, PaneId,
    PanesLayout, TabBarHoverIndex, TerminalPaneId,
};
...
pub struct Workspace {
    ...
    workspace_switcher_menu: ViewHandle<Menu<ProjectHostAction>>,
    show_workspace_switcher_menu: bool,
...
impl Workspace {
    ...
    fn build_settings_views(
        ...
        let screen_id = ctx.view_id();
        SettingsPaneManager::handle(ctx).update(ctx, |manager, _| {
            manager.register_view(screen_id, settings_pane.clone());
        });
    ...
    fn handle_remove_tab_config_confirmation_event(
        ...
        match event {
            RemoveTabConfigConfirmationEvent::Confirm { path } => {
                ...
                let is_removed_default = new_session_settings.default_session_mode()
                    == DefaultSessionMode::TabConfig
                    && new_session_settings.default_tab_config_path() == path.to_string_lossy();
                if is_removed_default {
                    NewSessionSettings::handle(ctx).update(ctx, |settings, ctx| {
                        report_if_error!(
                            settings
    ...
    pub fn new(
        ...
        ctx.subscribe_to_model(&autoupdate_handle, |_view, _handle, _evt, ctx| {
            ctx.notify();
        });
        ...
        let mut ws = Self {
            ...
            workspace_switcher_menu: Self::build_workspace_switcher_menu(ctx),
            show_workspace_switcher_menu: false,
    ...
    fn configure_new_workspace(
        ...
        match workspace_setting {
            ...
            NewWorkspaceSource::Restored {
                ...
                let screen = window_snapshot
                    .screen(screen_index)
                    .cloned()
                    .unwrap_or_default();
                let active_tab_index = screen.active_tab_index;
    ...
    /// The current focusable panels are: theme chooser, AI, and resource center (keyboard
    /// shortcuts page only)
    ...
    fn build_workspace_switcher_menu(
        ctx: &mut ViewContext<Self>,
    ) -> ViewHandle<Menu<ProjectHostAction>> {
        ...
        ctx.subscribe_to_view(&menu, |me, _, event, ctx| {
            if let MenuEvent::Close { .. } = event {
                me.show_workspace_switcher_menu = false;
                ctx.notify();
            }
        });
        menu
    ...
    pub fn show_workspace_switcher_dropdown(&mut self, ctx: &mut ViewContext<Self>) {
        if !FeatureFlag::AdeWorkspaces.is_enabled() {
            return;
        }
        let window_id = self.window_id;
        let open_project_ids: Vec<ProjectId> = WorkspaceRegistry::as_ref(ctx)
            .workspaces_for_window(window_id, ctx)
            .iter()
            .filter_map(|workspace| workspace.as_ref(ctx).project_id)
            .collect();
        let registry = ProjectRegistryModel::as_ref(ctx);
        let active_project_id = self.project_id;

        let mut items: Vec<MenuItem<ProjectHostAction>> = vec![
            MenuItemFields::new("Switch Workspace")
                .with_disabled(true)
                .into_item(),
        ];

        let mut home =
            MenuItemFields::new("Home").with_on_select_action(ProjectHostAction::ActivateHome);
        home = if active_project_id.is_none() {
            home.with_icon(icons::Icon::Check)
        } else {
            home.with_indent()
        };
        items.push(home.into_item());

        let open_projects: Vec<&Project> = open_project_ids
            .iter()
        ...
        let closed_projects: Vec<&Project> = registry
            .projects_mru()
            .into_iter()
            .filter(|project| !open_project_ids.contains(&project.id))
            .collect();
        if !closed_projects.is_empty() {
            items.push(MenuItem::Separator);
            for project in closed_projects {
                items.push(
                    MenuItemFields::new(project.display_name.clone())
                        .with_indent()
        ...
        self.workspace_switcher_menu
            .update(ctx, |menu, ctx| menu.set_items(items, ctx));
        self.show_workspace_switcher_menu = true;
        ctx.focus(&self.workspace_switcher_menu);
        ctx.notify();
    }

    fn render_workspace_switcher_pill(
        &self,
        appearance: &Appearance,
        ctx: &AppContext,
        ...
        let has_workspaces = self.project_id.is_some()
            || !ProjectRegistryModel::as_ref(ctx).is_empty()
            || WorkspaceRegistry::as_ref(ctx)
                .workspaces_for_window(self.window_id, ctx)
                .len()
                > 1;
        if !has_workspaces {
            return None;
        ...
        let label = self.workspace_switcher_label(ctx);
        ...
        .with_cursor(Cursor::PointingHand)
        .on_click(|ctx, _, _| {
            ctx.dispatch_typed_action(WorkspaceAction::ShowWorkspaceSwitcherMenu);
        })
        .finish();

        Some(
            Container::new(SavePosition::new(pill, WORKSPACE_SWITCHER_PILL_POSITION_ID).finish())
    ...
    fn unified_new_session_menu_items(
        ...
        let new_session_settings = NewSessionSettings::as_ref(ctx);
        let effective_default = new_session_settings.default_session_mode();
        let default_tab_config_path = new_session_settings.default_tab_config_path().to_string();
    ...
    fn should_trigger_get_started_onboarding(&self, ctx: &mut ViewContext<Self>) -> bool {
        ...
        if crate::root_view::has_completed_local_onboarding(ctx) {
            return false;
        }
    ...
    fn open_settings_pane(
        ...
        if let Some(locator) = settings_pane_manager.as_ref(ctx).find_pane(ctx.view_id()) {
    ...
    pub fn screen_snapshot(&self, window_id: WindowId, app: &AppContext) -> ProjectScreenSnapshot {
    ...
    fn launch_agent_from_picker(
        ...
        if let Some(locator) = picker_locator
            && let Some(tab) = self
                .tabs
                .iter()
    ...
    pub(crate) fn handle_file_tree_event(
        ...
        match event {
            ...
            pane_group::Event::ExecuteCommand(_) => {
                // Clear the task status indicator as soon as the user runs a command.
    ...
    fn update_active_session(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        match pane_group_handle.as_ref(ctx).active_session_view(ctx) {
            Some(terminal_handle) => {
                ...
                let window_id = ctx.window_id();
                let is_active_screen = WorkspaceRegistry::as_ref(ctx)
    ...
    pub fn focus_active_tab(&mut self, ctx: &mut ViewContext<Self>) {
    ...
    fn render_tab_bar_contents(
        ...
        if let Some(content_type) = self.get_simplified_wasm_tab_bar_content(ctx) {
            ...
            // Right: Info button + "View all cloud runs" button (for ambient agent sessions)
            let right_row = Flex::row()
                .with_cross_axis_alignment(CrossAxisAlignment::Center)
                .with_main_axis_size(MainAxisSize::Min);
    ...
    /// (update pill, offline indicator, settings, etc.) that are not configurable.
    fn add_configurable_right_side_tab_bar_controls(
        ...
        if let Some(pill) = self.render_workspace_switcher_pill(appearance, ctx) {
    ...
    fn banner_fields(&self, app: &AppContext) -> Option<WorkspaceBannerFields> {
        // The settings error banner takes priority — it's more important that users are
        // notified their settings file is broken than that they continue to see any of the
        // autoupdate or crash recovery banners.
        let banner_fields = self
            .render_settings_error_banner()
            ...
            match autoupdate::get_update_state(app) {
                ...
                | AutoupdateStage::UpdatedPendingRestart { new_version } => {
                    if is_incoming_version_past_current(new_version.soft_cutoff.as_deref()) {
                        ...
                        .update_by
                        .is_some_and(|update_by| chrono::Utc::now() > update_by)
                    {
                        Some(WorkspaceBannerFields {
                            banner_type: WorkspaceBanner::VersionDeprecated,
                            severity: BannerSeverity::Warning,
    ...
    fn add_toggle_setting_context_flags(&self, app: &AppContext, context: &mut Context) {
        let _privacy_settings = PrivacySettings::as_ref(app);
...
impl TypedActionView for Workspace {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            AddDefaultTab => {
                let effective_mode = NewSessionSettings::as_ref(ctx).default_session_mode();
...
impl View for Workspace {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if self.show_workspace_switcher_menu {
            stack.add_positioned_overlay_child(
                ChildView::new(&self.workspace_switcher_menu).finish(),
                OffsetPositioning::offset_from_save_position_element(
                    WORKSPACE_SWITCHER_PILL_POSITION_ID,
                    vec2f(0., 4.),
                    PositionedElementOffsetBounds::WindowByPosition,
                    PositionedElementAnchor::BottomLeft,
                    ChildAnchor::TopLeft,
            ...
            if let Some(sidecar_item) = &self.tab_config_action_sidecar_item {
                ...
                if let Some(anchor_label) = anchor_label {
                    let is_already_default = {
                        let new_session_settings = NewSessionSettings::as_ref(app);
                        let current_mode = new_session_settings.default_session_mode();
                        let current_path = new_session_settings.default_tab_config_path();
                        match sidecar_item {
...
impl Workspace {
    ...
    pub(crate) fn on_tab_drag(
        ...
        let cross_window_drag_allowed = self.project_id.is_none();

        if (is_drag_outside_tab_bar || source_is_single_tab)
            && cross_window_drag_allowed
